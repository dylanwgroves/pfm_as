...
/* Run Prelim File _____________________________________________________________ // Don't have to always re-run this but do have to re-run after an update	
	...
	do "${code}/../pfm_audioscreening/pfm_as_prelim_update.do"
...
estimates clear
	...
	foreach var of local dv {
		qui xi : regress `var' treat##c.resp_age ${cov_always}, cluster(id_village_n)
		estimates store sb_`var'
	}	
		
estimates table sb_*, keep(treat##c.resp_age) b(%7.4f) se(%7.4f)  p(%7.4f) stats(N r2_a)
